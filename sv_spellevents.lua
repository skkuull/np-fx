...
AddEventHandler("fx:spell:start", function(SpellCast, ptID, ptID2)
    ...
    TriggerParticalOnPlayer(SpellBaseStart["Lib"], SpellBaseStart["Part"], true, serveID, SpellBaseStart["Bone"], position, nil, players, ptID)
    RegisterParticalToggle(serverID, ptID, players, 1000 * 3)

    Wait(2500)
    if SpellBase["Display"] then
        position = SpellBase["Offsets"]
        TriggerParticalOnPlayer(SpellBaseStart["Lib"], SpellBaseStart["Part"], true, serverID, SpellBaseStart["Bone"], position, nil, players, ptID2)
        RegisterParticalToggle(serverID, ptID2, players, 1000 * 3)
    end
end)
...
AddEventHandler("fx:spell:target", function(pCoords,SpellCast)
    ...
    -- Trigger client event to effect players based on spell
end)
